...
class Utils:
    ...
    def getDashboardData(self) -> dict:
        # Falls der WebDriver bereits geschlossen wurde, verhindern wir Fehler
        if not self.webdriver:
            logging.error("[DASHBOARD] WebDriver ist bereits geschlossen, kann Dashboard-Daten nicht abrufen!")
            return {}

        try:
            urlBefore = self.webdriver.current_url  # Speichert aktuelle URL
            self.goToRewards()
            return self.webdriver.execute_script("return dashboard")  # Holt Dashboard-Daten

        except Exception as e:
            logging.error(f"[DASHBOARD ERROR] Konnte Dashboard-Daten nicht abrufen: {e}")
            return {}
...
def sendNotification(title: str, body: str, e: Exception = None) -> None:
    if not CONFIG.apprise.enabled or (e and not CONFIG.get("apprise.notify.uncaught-exception")):
        return
    ...
    try:
        ...
        if not urls:
            logging.error("[APPRISE ERROR] No Apprise URLs configured!")
            return

        for url in urls:
            apprise.add(url)

        success = apprise.notify(title=str(title), body=str(body))

        if not success:
            logging.error("[APPRISE ERROR] Notification failed! Check your Apprise URLs.")
    except Exception as e:
        logging.error(f"[APPRISE ERROR] Unexpected error: {e}")
