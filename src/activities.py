...
class Activities:
    ...
    def dashboardPopUpModalCloseCross(self):
        try:
            element = self.webdriver.find_element(By.XPATH,
                                                  "//div[@id=\'rewardsBanner\']/div/div/div[2]/div[4]/"
                                                  "mee-rewards-user-status-banner-item/mee-rewards-user-status-banner-streak/mee-rewards-pop-up/div/div[2]/div[3]/a")
    ...
    def completeQuiz(self):
        ...
        startQuiz.click()
        self.browser.utils.waitUntilVisible(By.XPATH, '//*[@id="currentQuestionContainer"]/div/div[1]', 4)
        time.sleep(2)
        numberOfQuestions = self.webdriver.execute_script("return _w.rewardsQuizRenderInfo.maxQuestions")
        numberOfOptions = self.webdriver.execute_script("return _w.rewardsQuizRenderInfo.numberOfOptions")
        for question in range(numberOfQuestions):
            if numberOfOptions == 8:
                answers = []
                for i in range(numberOfOptions):
                    isCorrectOption = self.webdriver.find_element(By.ID, f"rqAnswerOption{i}").get_attribute("iscorrectoption")
                ...
                for i in range(numberOfOptions):
                    if (self.webdriver.find_element(By.ID, f"rqAnswerOption{i}").get_attribute("data-option") == correctOption):
    ...
    def completeABC(self):
        ...
        for question in range(numberOfQuestions):
            self.webdriver.find_element(By.ID, f"questionOptionChoice{question}{random.randint(0, 2)}").click()
            time.sleep(1.5)
    ...
    def completeThisOrThat(self):
        ...
        startQuiz = self.browser.utils.waitUntilQuizLoads()
        startQuiz.click()
        self.browser.utils.waitUntilVisible(By.XPATH, '//*[@id="currentQuestionContainer"]/div/div[1]', 10)
        time.sleep(4)
        for _ in range(10):
            correctAnswerCode = self.webdriver.execute_script("return _w.rewardsQuizRenderInfo.correctAnswer")
    ...
    def getAnswerAndCode(self, answerId: str) -> tuple:
        ...
        if answerTitle is not None:
            return (answer,self.browser.utils.getAnswerCode(answerEncodeKey, answerTitle),)
        else:
