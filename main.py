...
def main():
    ...
    for currentAccount in loadedAccounts:
        try:
            ...
            Utils.sendNotification(f"⚠️ Error executing {currentAccount.username}, please check the log",f"{e1}\n{e1.__traceback__}",)
            continue
        ...
        logging.info(f"[POINTS] Data for '{currentAccount.username}' appended to the file.")
...
def log_daily_points_to_csv(earned_points, points_difference):
    ...
    new_row = {"Date": date,"Earned Points": earned_points,"Points Difference": points_difference,}
...
def setupLogging():
    ...
    logging.basicConfig(level=logging.DEBUG,format=_format,handlers=[
            handlers.TimedRotatingFileHandler(logs_directory / "activity.log",when="midnight",interval=1,backupCount=2,encoding="utf-8",),terminalHandler,],)
...
def argumentParser() -> argparse.Namespace:
    ...
    parser.add_argument("-p","--proxy",type=str,default=None,help="Optional: Global Proxy (ex: http://user:pass@host:port)",)
    parser.add_argument("-vn","--verbosenotifs",action="store_true",help="Optional: Send all the logs to the notification service",)
    parser.add_argument("-cv","--chromeversion",type=int,default=None,help="Optional: Set fixed Chrome version (ex. 118)",)
    parser.add_argument("-da","--disable-apprise",action="store_true",help="Optional: Disable Apprise, overrides config.yaml, useful when developing",)
    parser.add_argument("-t","--searchtype",type=str,default=None,help="Optional: Set to only search in either desktop or mobile (ex: 'desktop' or 'mobile')",)
...
def setupAccounts() -> list[Account]:
    ...
    if not accountPath.exists():
        accountPath.write_text(json.dumps(
            [{"username": "Your Email", "password": "Your Password"}], indent=4),encoding="utf-8",)
        noAccountsNotice = """
...
def executeBot(currentAccount: Account, args: argparse.Namespace):
    ...
    logging.info(f"[POINTS] You are now at {Utils.formatNumber(accountPoints)} points !"+f"remaining Searches: {remainingSearches}")

    appriseSummary = AppriseSummary[Utils.loadConfig().get("apprise", {}).get("summary", AppriseSummary.ALWAYS.name)]
    if appriseSummary == AppriseSummary.ALWAYS:
        ...
        Utils.sendNotification("Daily Points Update","\n".join([f"👤 Account: {currentAccount.username}",f"⭐️ Points earned today: {Utils.formatNumber(accountPoints - startingPoints)}",f"💰 Total points: {Utils.formatNumber(accountPoints)}",goalStatus,]),)



    elif appriseSummary == AppriseSummary.ON_ERROR:
        if remainingSearches.getTotal() > 0:
            Utils.sendNotification("Error: remaining searches",f"account username: {currentAccount.username}, {remainingSearches}",)
...
def load_previous_points_data():
    try:
        with open(Utils.getProjectRoot() /"logs"/"previous_points_data.json", "r") as file:
...
if __name__ == "__main__":
    try:
        ...
        #logging.exception("")
        Utils.sendNotification("⚠️ Error occurred, please check the log", f"{e}\n{e.__traceback__}")
